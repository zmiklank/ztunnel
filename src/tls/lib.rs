...
//! TLS configuration and certificate management for ztunnel.
//!
//! This module provides:
//! - `TlsProfile` and `TlsVersion` for runtime TLS configuration via xDS
//! - `provider()` function to create rustls CryptoProvider instances
//! - `tls_versions()` to get supported TLS protocol versions
//! - Traits for certificate providers

// Re-export public API from sibling modules
pub use super::profile::{TlsProfile, TlsVersion};
pub use super::provider::{provider, tls_versions, CRYPTO_PROVIDER};
...
/// Provider for control plane client certificates.
...
/// Provider for server certificates.
#[async_trait::async_trait]
